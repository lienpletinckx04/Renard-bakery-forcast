...
### 2026-09-18, Too Good To Go volledig uit scope: kanaal, code en historische data verwijderd

**Wat:** Too Good To Go is niet langer een kanaal van het platform. Verwijderd, in deze volgorde: de historische omzet in productie (migratie `017_tgtg_verwijderd.sql` — 1.872 rijen in `fact_verkoop`, 4 juli 2019 t/m 31 juli 2026, € 106.415,75, en alle 83 rijen in `fact_kanaalkost`; gemeten vóór verwijdering), de `kanaal`-CHECK-constraints op `fact_verkoop` en `fact_kanaalkost` (nu `winkel`/`deliveroo`/`overig`), de parser (`bakkerij/sources/tgtg_parse.py`) en alle extractie-/analysescripts (`scripts/tgtg_extract.py`, `scripts/tgtg_restwaarde.py`, `scripts/kassa_analyse.py` — dat laatste was een eenmalige analyse die op de tgtg-data leunde om te bewijzen dat de drie kassa's samen tellen; die vraag is al beantwoord), elke tgtg-tak in de berekenings- en contractlaag (`canoniek.py`, `berekening.py`, `contract.py`, `briefing.py`, `kwaliteit.py`, `io_load.py`, `taal.py`, `opmaak.py`, `audit/profile.py`), het bevriezingsmechanisme dat het tgtg-kanaal na het schrappen van blok 9 uit de database teruglas (`bakkerij/db/bevroren.py`, uitsluitend voor tgtg gebouwd en dus in zijn geheel verwijderd, met zijn test), en de frontend (`platform/lib/*`, het kanalenscherm). De generieke mechanismen die tgtg meebediende — de bevroren-routering in `kwaliteit.py`, de krimpwacht in `contract_rijen.py` — blijven bestaan voor een volgend kanaal dat ze nodig heeft; alleen hun tgtg-instantie is weg.
...
### 2026-09-18, Het openingsvenster van de periodekiezer staat in het contract, niet in de volgorde van de lijst

**Wat:** de periodekubus krijgt een kort venster van zeven gemeten open dagen (`d7`), vooraan in de lijst, en de vensters dragen sindsdien een optionele vlag `standaard` die zegt welk venster de kiezer opent. Die vlag staat op `d30`. De knoprij loopt daarmee van kort naar lang terwijl het openingsbeeld dertig dagen blijft.
**Waarom:** de opdrachtgever maakt zijn eigen managementrapporten per week op (zie het Fort Jaco-rapport van september 2026: week 1 naast week 2, met klanten, POS, Deliveroo en gemiddeld ticket per dag) en kon die naast dit platform niet leggen — de kiezer bood alleen 30 dagen, 13 weken, 12 maanden en een jaar. Een week erbij is één venster, geen nieuwe laag. Maar `PeriodePaneel` opende tot nu toe `vensters[0]`, en dan zou een venster vooraan zetten stilzwijgend het openingsbeeld van het hele dashboard veranderen naar zeven dagen — precies wat de opdrachtgever níét wil ("zeven dagen is te kort"). Volgorde en openingsvenster zijn twee verschillende vragen en horen dus niet op één veld te liften.
**Gevolg:** `d7` telt gemeten open dagen en geen kalenderdagen, zoals elk ander dagvenster hier. Bij een zaak die zes dagen per week draait beslaat het dus ruim een kalenderweek, en na een sluiting meer. Dat staat in de toelichting van het venster zelf (die het gevónden aantal draagt, terwijl het knoplabel het gevráágde draagt), want een "week" die stilzwijgend acht kalenderdagen beslaat is het soort verschil dat niemand naast elkaar legt tot het te laat is.
**Alternatief 1:** `d7` achteraan de lijst zetten. Verworpen: de knoprij is een schaal, en een venster dat uit de schaal springt leest als een fout in het scherm.
**Alternatief 2:** de kiezer de kortste periode laten openen. Verworpen om de reden hierboven; bovendien is een week te weinig grond om een dashboard mee te openen, ook nu de lezer zelf kan wisselen.
**Alternatief 3:** een vrije datumkiezer in plaats van vaste vensters. Verworpen voor fase 1: de kubus is bewust voorgebakken (harde regel 4 — het scherm rekent niet), en een vrij bereik betekent rekenen op verzoek. Genoteerd als kandidaat zodra er vraag naar is.

---

### 2026-09-19, Deliveroo komt binnen op dagniveau uit Orders, via de postbus, als periodiek kanaal

**Wat:** het kanaal Deliveroo staat in `fact_verkoop` met per dag per vestiging één rij op het synthetische product `dl-bestellingen` (`aantal` = bestellingen, `omzet_excl_btw` = subtotaal min commissie), en in `fact_kanaalkost` per maand met gemeten gemiddelden. De bron zijn de Orders-exports uit Partner Hub, die op de runner uit de postbus (`bron_upload`) komen: de nachtelijke ketting haalt ze elke nacht op zonder af te vinken en bouwt het canonieke model van nul. Het kanaal is uit `BEKEND_AFWEZIG` en staat in `ACHTER_DAGEN_PER_KANAAL` met een lat van 90 dagen. `config/winkels.json` kent de Deliveroo-vestigingsnamen naast de Odoo-kassa's, zodat de winkelweergave het kanaal meeneemt.
**Waarom dagniveau en geen artikelniveau:** de echte Items Sold-export draagt geen datum en geen bestelnummer — hij telt op over de hele downloadperiode van ~16 dagen. De beslissing van 25 augustus (een `dl-`-product per artikel, gekoppeld via een ordersleutel) ging uit van een vorm die Deliveroo niet levert, en vervalt. Wat er wél per dag bestaat, is de bestelling zelf, mét commissie; dat is precies wat het kanalenscherm en de dagtabel nodig hebben. Een omzet per artikel per dag zou een verdeling van een periodetotaal zijn, en die hoort niet stilzwijgend in een laadstap.
**Waarom netto in `omzet_excl_btw`:** het precedent van 28 augustus, ongewijzigd. De btw op de commissie is aftrekbaar en dus geen kost; het btw-regime van het subtotaal zelf blijft onbevestigd, en dat voorbehoud staat op het kanalenscherm.
**Waarom de postbus en niet git of een schijf:** de runner onthoudt niets tussen twee nachten, klantdata hoort niet in git (harde regel 2), en Partner Hub geeft de historiek na twaalf maanden niet meer. De postbus is dus niet alleen de aanvoer maar het archief. Daarom vinkt de ketting niets af: een afgevinkte rij wordt niet meer opgehaald, en de tweede nacht zou zonder Deliveroo bouwen. `LEZERS` in `uploads_verwerk.py` blijft leeg, met opzet.
