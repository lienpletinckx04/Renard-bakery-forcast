...
|---|---|
| `make tgtg` kan bij een kapotte omgeving een lege CSV met exitcode 0 achterlaten; `make extract` is stil bij nul rijen; geen netwerktimeout op de Odoo-client (het lege extract is 18 aug gedicht; de rest staat nog) | De oorspronkelijke reden om te wachten — "dit wordt pas operationeel bij S1/S7-S9" — is half vervallen: S7-S9 zijn op 17 aug geschrapt, en daarmee is het handmatige pad (`make tgtg`, het uploadvangnet) niet langer een vangnet maar **de enige TGTG-aanvoer**. Dat verdient een nieuwe afweging: een hardening-pas op de handmatige keten is nu meer waard dan toen deze rij geschreven werd |
...
| ~~De runner mist de TGTG-bestanden~~ | ✅ **19 aug gedaan.** Het kanaal is bevroren, dus de database is er de bewaarplaats van: `bakkerij/db/bevroren.py`, met een heen-en-terugtest tegen een echte Postgres en een meting tegen de echte Supabase (1.872 regels, rij voor rij gelijk). Wat nog wél ontbreekt op een runner is `sluitingsdagen.json` — zie `todo.md` |
