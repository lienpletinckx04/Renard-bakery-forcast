...
|---|---|
| **`fact_bonnen` draagt preprod-kassanamen en stopt op 7 augustus** | **Nieuw, 18 sep 2026, en het dringendste punt van deze lijst.** De verkoop komt sinds vanavond uit de productie-Odoo (`Ixelles Kassa 1/2/3`), maar `fact_bonnen` kent alleen nog de preprodnamen (`Kassa 1/2/3`). Gevolg: het bonnenaantal en het gemiddelde bonbedrag staan op 7 augustus terwijl de omzet ernaast tot vandaag loopt, en `config/winkels.json` herkent die oude namen niet, dus per winkel valt er niets toe te wijzen. Oplossing: één run van `make extract-bonnen` met de productie-`.env`, gevolgd door de laadstap. Waarom het niet vanzelf meeging: `odoo_bonnen.py` staat bewust buiten de nachtelijke ketting omdat het anderhalf miljoen bonregels herleest — zie de kop van `scripts/nachtelijke_sync.py`. Zolang dit openstaat, draagt één scherm twee snelheden |
